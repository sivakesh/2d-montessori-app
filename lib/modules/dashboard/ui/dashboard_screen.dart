...
class _DashboardScreenState extends ConsumerState<DashboardScreen> {
  ...
  MoodCheckinModel? _selfLatestCheckIn;
  ...
  Future<void> _loadDashboardData() async {
    ...
    try {
      ...
      final selfLatestCheckIn = currentUser == null
          ? null
          : await moodService.getLatestMoodForEntity('staff', currentUser.id);
      ...
      setState(() {
        ...
        _selfLatestCheckIn = selfLatestCheckIn;
      });
    } catch (_) {
      if (!mounted) return;
      setState(() {
  ...
  Future<void> _showManualStaffCheckIn() async {
    ...
    await _saveManualMoodCheckIn(
      ...
      entityName: currentUser.name?.isNotEmpty == true ? currentUser.name! : currentUser.phone,
      createdBy: currentUser.id,
    );
  }
        ...
        child: Column(
          ...
          children: [
            _DashboardHeader(
              ...
              _SelfCheckInCard(
                latest: _selfLatestCheckIn,
              isSaving: _savingCheckin,
              onCheckIn: _savingCheckin
                  ? null
                  : () async {
                      final currentUser = ref.read(currentUserProvider);
                      if (currentUser == null) return;
                      await _saveManualMoodCheckIn(
                        entityType: 'staff',
                        entityId: currentUser.id,
                        entityName: currentUser.name?.isNotEmpty == true
                            ? currentUser.name!
                            : currentUser.phone,
                        createdBy: currentUser.id,
                      );
            ...
            if (_loadingMetrics)
              ...
              if (_recentMoodCheckins.isEmpty)
                ...
                Column(
                  children: [
                    for (final item in _recentMoodCheckins)
                      Padding(
...
class _SelfCheckInCard extends StatelessWidget {
  const _SelfCheckInCard({
    required this.latest,
    required this.isSaving,
    required this.onCheckIn,
  });

  final MoodCheckinModel? latest;
  final bool isSaving;
  ...
  Widget build(BuildContext context) {
    ...
    final timeLabel = time == null ? null : TimeOfDay.fromDateTime(time).format(context);
    return Card(
      elevation: 0,
      shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)),
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            if (latest != null) ...[
              const SizedBox(height: 14),
              Text(
                'Last check-in: ${latest!.moodLabel}${timeLabel == null ? '' : ' · $timeLabel'}',
            ...
            SizedBox(
              ...
              child: FilledButton(
                onPressed: isSaving ? null : onCheckIn,
                child: Text(isSaving ? 'Saving...' : 'Check In'),
              ),
            ),
          ],
        ),
