...
abstract final class PublicPageViewMapper {
  static PublicPageView fromSnapshot(
    DocumentSnapshot<Map<String, dynamic>> snapshot,
  ) {
    final data = snapshot.data() ?? const {};
    return PublicPageView(
      pageId: data['pageId'] as String? ?? snapshot.id,
      ...
      resolvedRelatedPages: {
        for (final entry
            in ((data['resolvedRelatedPages'] as Map?) ?? const {}).entries)
          entry.key as String: RelatedPageSummary.fromMap(
