...
"Phase 1 Foundation" and "Phase 1 — CMS Core: `feature_publishing`"
sections for what each milestone added and what remains genuinely
unverified (emulator-dependent tests not yet covered by a CI run — see
"Testing" below).
...
packages/            core_contracts, design_system, firebase_adapters,
                      ...
                      feature_publishing (workflow engine domain/data, Phase 1 CMS Core —
                      no presentation layer yet), and 14 other feature_* packages —
                      see repository-structure.md
functions/            Cloud Functions (TypeScript) — src/auth and src/publishing are
                      implemented; the rest (media, enquiries, ...) are stubs
firebase/             Firestore/Storage rules (users/{uid}, content/{contentId} +
                      transitions/, and auditLogs are live), indexes, emulator
                      fixtures, and firebase/test/ (rules + auth-emulator tests)
...
# against real Auth + Firestore emulators
